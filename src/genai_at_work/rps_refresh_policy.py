...
def _strings(mapping: Mapping[str, Any], key: str, *, context: str) -> list[str]:
    ...
    if not isinstance(value, list) or not all(
        ...
        raise RpsRefreshPolicyError(
            f"{context}.{key} must be a list of non-empty strings"
        )
    return list(value)
...
def _validate_activation_evidence(policy: Mapping[str, Any]) -> set[str]:
    ...
    if statuses["successful_live_validation"] == "passed":
        ...
        artifact_digest = _string(
            live, "artifact_digest", context=live_context
        ).lower()
        ...
        if (
            _string(live, "revision_status", context=live_context)
            not in _ALLOWED_REFRESH_STATUSES
        ):
            raise RpsRefreshPolicyError(
                "successful_live_validation.revision_status is unsupported"
            )
        provider_count = _positive_int(
            live, "provider_series_count", context=live_context
        )
        observatory_count = _positive_int(
            live, "observatory_series_count", context=live_context
        )
        excluded_count = _nonnegative_int(
            live, "excluded_series_count", context=live_context
        )
...
def validate_rps_refresh_policy(policy: Mapping[str, Any]) -> None:
    ...
    if _string(policy, "policy_status", context="policy") != "PINNED_NOT_ACTIVATED":
        raise RpsRefreshPolicyError(
            "RPS refresh policy must remain PINNED_NOT_ACTIVATED until gates pass"
        )

    evidence = _mapping(policy.get("evidence_basis"), context="policy.evidence_basis")
    if (
        _string(evidence, "source_frequency", context="policy.evidence_basis")
        != "quarterly"
    ):
        raise RpsRefreshPolicyError(
            "Registered RPS source frequency must remain quarterly"
        )
    if (
        _string(
            evidence,
            "provider_next_release_date_status",
            context="policy.evidence_basis",
        )
        != "not_available"
    ):
        raise RpsRefreshPolicyError(
            "Pinned cadence assumes no provider next-release date is available"
        )
    ...
    if (
        _TIME_RE.fullmatch(
            _string(check, "time_utc", context="policy.source_check")
        )
        is None
    ):
    ...
    if not isinstance(lag, int) or isinstance(lag, bool) or lag != 7:
        raise RpsRefreshPolicyError(
            "Weekly RPS checks require maximum_nominal_detection_lag_days=7"
        )
    if (
        _string(check, "schedule_activation", context="policy.source_check")
        != "deferred"
    ):
        raise RpsRefreshPolicyError(
            "Scheduled RPS retrieval must remain deferred until activation gates pass"
        )
    gates = set(
        _strings(check, "activation_requirements", context="policy.source_check")
    )
    ...
    unchanged = _mapping(
        policy.get("unchanged_source"), context="policy.unchanged_source"
    )
    for key in (
        ...
        if _bool(unchanged, key, context="policy.unchanged_source"):
            raise RpsRefreshPolicyError(
                f"Unchanged-source policy must keep {key}=false"
            )
    if not _bool(
        unchanged, "retain_review_safe_check_evidence", context="policy.unchanged_source"
    ):
        raise RpsRefreshPolicyError(
            "Unchanged checks must retain review-safe check evidence"
        )
    ...
    if statuses != {"baseline", "new_wave", "revision", "mixed"}:
        raise RpsRefreshPolicyError(
            "Changed-source statuses must exactly cover baseline/new_wave/revision/mixed"
        )
    for key in (
    ...
    if _bool(publication, "automatic", context="policy.publication"):
        raise RpsRefreshPolicyError(
            "RPS publication may never be automatic under this policy"
        )
    for key in (
        "requires_ci",
        "requires_exact_staged_hash_review",
        "requires_global_observatory_release_composition",
    ):
        if not _bool(publication, key, context="policy.publication"):
            raise RpsRefreshPolicyError(
                f"Publication policy requires {key}=true"
            )
...
def _change_counts(summary: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    for key in (
        ...
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise RpsRefreshPolicyError(
                f"summary.change_counts.{key} must be a non-negative integer"
            )
    return counts
...
def action_for_refresh_summary(
    ...
    if status == "unchanged":
        if total_changes != 0 or summary.get("requires_release_review") is not False:
            raise RpsRefreshPolicyError(
                "Unchanged summary contradicts its change counts/review flag"
            )
        return {
    ...
    if summary.get("requires_release_review") is not True:
        raise RpsRefreshPolicyError(
            "Changed/baseline source summary must require release review"
        )
...
def activation_gates_satisfied(policy: Mapping[str, Any]) -> bool:
    """Return whether every activation gate has validated recorded evidence."""

    return recorded_activation_gates(policy) == _REQUIRED_ACTIVATION_GATES
